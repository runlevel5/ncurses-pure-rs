...
/// Check if a sequence looks like a CSI u sequence.
/// CSI u format: \x1b [ {digits} ; {digits} u
fn is_csi_u_sequence(data: &[u8]) -> bool {
    // Minimum: \x1b [ digit ; digit u = 6 bytes
    if data.len() < 6 {
        return false;
    }
    // Must start with \x1b[
    ...
    // Must end with 'u'
    if data[data.len() - 1] != b'u' {
        return false;
    }
    // Middle part should be digits and semicolon
    let middle = &data[2..data.len() - 1];
    let has_semicolon = middle.contains(&b';');
    let valid_chars = middle
        .iter()
        .all(|&b| b.is_ascii_digit() || b == b';' || b == b':');
    has_semicolon && valid_chars
}

/// Check if a sequence could be the start of a CSI u sequence.
/// This is used to keep accumulating bytes before the escape parser rejects them.
/// CSI u format: \x1b [ {digits} ; {digits} u
fn is_potential_csi_u_prefix(data: &[u8]) -> bool {
    // Must start with \x1b[
    if data.len() < 3 || data[0] != 0x1b || data[1] != b'[' {
        return false;
    }
    // After \x1b[, CSI u sequences start with a digit (the codepoint)
    // Arrow keys and other sequences start with letters (A, B, C, D, etc.)
    // So we only consider it a potential CSI u if the first char after [ is a digit
    let middle = &data[2..];
    if middle.is_empty() {
        return false; // Just \x1b[ - let escape parser handle it
    }
    // First character after \x1b[ must be a digit for CSI u
    if !middle[0].is_ascii_digit() {
        return false;
    }
    // Check if all characters after \x1b[ are valid CSI u content
    // Valid: digits, semicolon, colon
    // The sequence ends with 'u', but we haven't seen it yet
    for &b in middle {
        if !b.is_ascii_digit() && b != b';' && b != b':' {
            return false;
        }
    }
    // We have \x1b[ followed by digits (and possibly ; or :), keep accumulating
    true
}

/// Parse a CSI u (fixterms/kitty) keyboard sequence and return the key code.
/// Format: \x1b [ codepoint ; modifiers u
/// Modifiers: 1=shift, 2=alt, 4=ctrl, 8=meta (actual value is modifier+1)
///
/// Returns the key code with modifiers applied, or None if parsing fails.
fn parse_csi_u_sequence(data: &[u8]) -> Option<i32> {
    if !is_csi_u_sequence(data) {
        return None;
    }

    // Extract the middle part between '[' and 'u'
    let middle = &data[2..data.len() - 1];
    let middle_str = std::str::from_utf8(middle).ok()?;

    // Split by semicolon
    let parts: Vec<&str> = middle_str.split(';').collect();
    if parts.len() < 2 {
        return None;
    }

    // Parse codepoint (first part)
    let codepoint: u32 = parts[0].parse().ok()?;

    // Parse modifiers (second part, may have additional ':' separated values)
    let mod_part = parts[1].split(':').next()?;
    let modifiers: u32 = mod_part.parse().ok()?;

    // Modifiers in CSI u are 1-indexed, so subtract 1
    let mod_flags = modifiers.saturating_sub(1);
    let shift = (mod_flags & 1) != 0;
    let _alt = (mod_flags & 2) != 0;
    let ctrl = (mod_flags & 4) != 0;
    let _meta = (mod_flags & 8) != 0;

    // Convert codepoint to key code
    // For control characters, the codepoint is the base character
    if ctrl && codepoint >= 0x40 && codepoint <= 0x7F {
        // Ctrl+letter produces control character (codepoint & 0x1F)
        // e.g., Ctrl+I (codepoint 73 or 105) -> 0x09 (TAB)
        let base_char = if codepoint >= 0x61 && codepoint <= 0x7A {
            // Lowercase letter, convert to uppercase for control calculation
            codepoint - 0x20
        } else {
            codepoint
        };
        let ctrl_char = (base_char & 0x1F) as i32;
        return Some(ctrl_char);
    }

    // For shifted letters, convert to uppercase
    if shift && codepoint >= 0x61 && codepoint <= 0x7A {
        return Some((codepoint - 0x20) as i32);
    }

    // Return the codepoint as-is for other cases
    Some(codepoint as i32)
}
...
impl Screen {
    ...
    fn parse_escape_sequence(&mut self) -> Result<i32> {
        ...
        loop {
            ...
            // Check for CSI u (fixterms/kitty) keyboard sequences: \x1b[{codepoint};{modifiers}u
            // These are sent by modern terminals like Ghostty, Kitty for modified keys
            // Format: CSI codepoint ; modifiers u
            // Modifiers: 1=shift, 2=alt, 4=ctrl, 8=meta (values are modifier+1)
            if byte == b'u' && sequence_buf.len() >= 4 && sequence_buf[1] == b'[' {
                if let Some(key) = parse_csi_u_sequence(&sequence_buf) {
                    return Ok(key);
                }
                // If we couldn't parse it but it looks like CSI u, consume it silently
                // to prevent the 'u' character from leaking through
                if is_csi_u_sequence(&sequence_buf) {
                    // Return ESC to signal an unrecognized escape sequence
                    return Ok(0x1b);
                }
            }

            // Check if we might be in the middle of a CSI u sequence
            // CSI u format: \x1b [ {digits} ; {digits} u
            // We need to keep accumulating if we see: \x1b [ followed by digits, semicolons, or colons
            // and we haven't seen a terminator yet
            if is_potential_csi_u_prefix(&sequence_buf) {
                // Don't feed to escape parser yet, keep accumulating
                // Set a reasonable limit to prevent infinite accumulation
                if sequence_buf.len() < 20 {
                    continue;
                }
            }
